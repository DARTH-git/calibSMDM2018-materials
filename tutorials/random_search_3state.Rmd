---
title: "Random Search Calibration"
...
- Model: Cancer Relative Survival Model
- Parameters to be calibrated: `p.Mets`, `p.DieMets`
- Targets: Surv
...
This (hypothetical) calibration target is the proportion of people that survive over time. 
...
In this case, the goodness of fit function is the likelihood of observing the model results, given the target data. We assume a normal likelihood for this example. In many cases, it's more numerically stable to use the log-likelihood `log = TRUE` and sum them together.
