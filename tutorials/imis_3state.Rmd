...
The IMIS algorithm has been used to calibrate health policy models [@Raftery2010a, @Menzies2017]
...
As before, we can use more than one target in the calibration. Insteady of definining a likelihood vector, we would want to define a likelihood *matrix*. Then, we can sum the $i$th of the matrix to get a total likelihood for the the $i$th parameter set.
